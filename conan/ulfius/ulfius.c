...
static int ulfius_is_valid_endpoint(const struct _u_endpoint * endpoint, int to_delete) {
  printf("ulfius_is_valid_endpoint() entered\n");
  printf("NOTE: Ulfius compiled at %s.\n", __TIMESTAMP__);

  if (endpoint != NULL) {
    printf("ulfius_is_valid_endpoint() if entered\n");
    if (ulfius_equals_endpoints(endpoint, ulfius_empty_endpoint())) {
      printf("ulfius_is_valid_endpoint() if branch: 1\n");
      // Should be the last endpoint of the list to close it
      return 1;
    } else if (endpoint->http_method == NULL) {
      printf("ulfius_is_valid_endpoint() if branch: 2\n");
      return 0;
    } else if (!to_delete && endpoint->callback_function == NULL) {
      printf("ulfius_is_valid_endpoint() if branch: 3\n");
      return 0;
    } else if (endpoint->url_prefix == NULL && endpoint->url_format == NULL) {
      printf("ulfius_is_valid_endpoint() if branch: 4\n");
      return 0;
    } else {
      printf("ulfius_is_valid_endpoint() if branch: 5\n");
      return 1;
    }
    printf("ulfius_is_valid_endpoint() if left\n");
  } else {
    return 0;
  }

  printf("ulfius_is_valid_endpoint() left\n");
}
...
int ulfius_add_endpoint(struct _u_instance * u_instance, const struct _u_endpoint * u_endpoint) {
  ...
  printf("ulfius_add_endpoint() entered\n");
  
  if (u_instance != NULL && u_endpoint != NULL) {
    
    printf("u_instance and u_endpoint is not NULL, continuing\n");

    if (ulfius_is_valid_endpoint(u_endpoint, 0)) {
      printf("ulfius endpoint is valid\n");

      if (u_instance->endpoint_list == NULL) {

        printf("u_instance->endpoint_list is NULL\n");
        ...
        printf("u_instance->endpoint_list IS NOT NULL\n");
...
int ulfius_equals_endpoints(const struct _u_endpoint * endpoint1, const struct _u_endpoint * endpoint2) {
  printf("ulfius_equals_endpoints() entered\n");

  if (endpoint1 != NULL && endpoint2 != NULL) {
    printf("ulfius_equals_endpoints() - outside if entered\n");
    if (endpoint1 == endpoint2) {
      printf("ulfius_equals_endpoints() - outside if - branch 1\n");
      return 1;
    } else if (o_strcmp(endpoint2->http_method, endpoint1->http_method) != 0) {
        printf("ulfius_equals_endpoints() - outside if - branch 2\n");
        return 0;
    } else if (o_strcmp(endpoint2->url_prefix, endpoint1->url_prefix) != 0) {
        printf("ulfius_equals_endpoints() - outside if - branch 3\n");
        return 0;
    } else if (o_strcmp(endpoint2->url_format, endpoint1->url_format) != 0) {
        printf("ulfius_equals_endpoints() - outside if - branch 4\n");
        return 0;
    } else {
      printf("ulfius_equals_endpoints() - outside if - branch 5\n");
      return 1;
    }
  } else {
    return 1;
...
int ulfius_add_endpoint_by_val(struct _u_instance * u_instance,
  ...
  printf("ulfius_add_endpoint_by_val()\n");
